...
SED 시스템의 활성 버전을 그대로 추출했다(2026-08-18 기준).
...
057·033은 프로젝트에 이미 있는 메시지를 재사용한 것이므로, 다른 시스템에서는
해당 시스템의 기존 메시지 번호로 바꾼다.
...
클래스에는 설계 검토 단계에서 만들었다가 폐기한 `GET_DEFAULT_OBJECT`가 남아
있다(호출부는 주석 처리). 필드명으로 권한 오브젝트를 자동 결정하려던 방식이며,
같은 필드라도 업무 문맥에 따라 오브젝트가 달라져 폐기했다. 실제 시스템 상태를
그대로 옮긴 것이므로 그대로 두었고, 신규 구축 시에는 생략해도 된다.
데모5의 `build_check`에도 같은 이유로 주석 처리된 구버전 블록이 있다.
